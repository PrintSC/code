...
from models.utils import LOGGER, TQDM, checks, clean_url, emojis, is_online, url2file

# Define models GitHub assets maintained at https://github.com/models/assets
GITHUB_ASSETS_REPO = "models/assets"
GITHUB_ASSETS_NAMES = (
...
def delete_dsstore(path, files_to_delete=(".DS_Store", "__MACOSX")):
    ...
    Example:
        ```python
        from models.utils.downloads import delete_dsstore
...
def zip_directory(directory, compress=True, exclude=(".DS_Store", "__MACOSX"), progress=True):
    ...
    Example:
        ```python
        from models.utils.downloads import zip_directory
...
def unzip_file(file, path=None, exclude=(".DS_Store", "__MACOSX"), exist_ok=False, progress=True):
    ...
    Example:
        ```python
        from models.utils.downloads import unzip_file
...
def check_disk_space(url="https://models.com/assets/coco8.zip", path=Path.cwd(), sf=1.5, hard=True):
    """
    Check if there is sufficient disk space to download and store a file.

    Args:
        url (str, optional): The URL to the file. Defaults to 'https://models.com/assets/coco8.zip'.
...
def get_google_drive_file_info(link):
    ...
    Example:
        ```python
        from models.utils.downloads import get_google_drive_file_info
...
def safe_download(
    ...
    Example:
        ...
        link = "https://models.com/assets/bus.jpg"
    ...
    if "://" not in str(url) and Path(url).is_file():  # URL exists ('://' check required in Windows Python<3.10)
        ...
        uri = (url if gdrive else clean_url(url)).replace(  # cleaned and aliased url
            "https://github.com/models/assets/releases/download/v0.0.0/",
            "https://models.com/assets/",  # assets alias
        )
...
def get_github_assets(repo="models/assets", version="latest", retry=False):
    """
    Retrieve the specified version's tag and assets from a GitHub repository. If the version is not specified, the
    function fetches the latest release assets.

    Args:
        repo (str, optional): The GitHub repository in the format 'owner/repo'. Defaults to 'models/assets'.
    ...
    Example:
        ```python
        tag, assets = get_github_assets(repo="models/assets", version="latest")
        ```
    """
    if version != "latest":
...
def attempt_download_asset(file, repo="models/assets", release="v8.3.0", **kwargs):
    """
    ...
    Example:
        ```python
        file_path = attempt_download_asset("yolo11n.pt", repo="models/assets", release="latest")
        ```
    """
    from models.utils import SETTINGS  # scoped for circular import
...
def download(url, dir=Path.cwd(), unzip=True, delete=False, curl=False, threads=1, retry=3, exist_ok=False):
    ...
    Example:
        ```python
        download("https://models.com/assets/example.zip", dir="path/to/dir", unzip=True)
        ```
    """
    dir = Path(dir)
