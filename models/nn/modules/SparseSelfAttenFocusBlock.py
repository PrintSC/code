...
class SparseAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        mask = torch.eye(H * W, device=x.device).bool().unsqueeze(0).unsqueeze(0)  # basic diagonal mask
        attn = attn.masked_fill(mask, float('-inf'))  # sparse mask (self can be masked)
...
class SparseFocusBlock(nn.Module):
    ...
    def forward(self, x, ctx=None):
        ...
        fused = self.norm1(fused + self.sparse_attn(fused))
